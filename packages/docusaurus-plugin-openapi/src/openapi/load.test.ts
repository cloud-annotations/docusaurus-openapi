...
import { _loadOpenapi, _newLoadOpenapi } from "./load";
import { OpenApiObjectWithRef } from "./types";

const example: OpenApiObjectWithRef = {
  openapi: "3.0.3",
      ...
      put: {
        ...
        parameters: [
          ...
          },
          {
            ...
            in: "header",
            description:
              "This header is used to specify the algorithm and key size to use with the encryption key stored by using Key Protect. This value must be set to the string `AES256`.",
            required: false,
            schema: { type: "string" },
          },
          {
            ...
            in: "header",
            description:
              "This header is used to reference the specific root key used by Key Protect or Hyper Protect Crypto Services to encrypt this bucket. This value must be the full CRN of the root key.",
            required: false,
            schema: { type: "string" },
          },
        ],
...
it("hello world", async () => {
  const expected = await _loadOpenapi(
    JSON.parse(JSON.stringify(example)),
    "@<baseUrl>",
    "@<routeBasePath>"
  );
  const actual = await _newLoadOpenapi(
    JSON.parse(JSON.stringify(example)),
    "@<baseUrl>",
    "@<routeBasePath>"
  );
